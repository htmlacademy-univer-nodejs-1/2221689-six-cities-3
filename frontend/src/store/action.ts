...
import { adaptCommentToClient, adaptCommentsToClient, adaptFullOfferToClient, adaptOffersToClient } from '../utils/adapters/adaptersToClient';
import { adaptCreateCommentToServer, adaptCreateOfferToServer, adaptSignupToServer } from '../utils/adapters/adaptersToServer';
...
export const registerUser = createAsyncThunk<void, UserRegister, { extra: Extra }>(
  ...
  async ({ email, password, name, avatar, isPro }, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<{id: string }>(ApiRoute.Register, adaptSignupToServer({ email, password, name, isPro }));
    if (avatar) {
